...
User1 = User(name="Robo Barista", email="tinnyTim@udacity.com",
             picture='https://pbs.twimg.com/profile_images/2671170543/18debd694\
             829ed78203a5a36dd364160_400x400.png')
session.add(User1)
session.commit()
...
menuItem4 = MenuItem(user_id=1, name="Sirloin Burger", description="Made\
                     with grade A beef",
...
menuItem5 = MenuItem(user_id=1, name="Root Beer", description="16oz\
...
menuItem7 = MenuItem(user_id=1, name="Grilled Cheese Sandwich",
                     description="On texas toast with American\
                     Cheese", price="$3.49", course="Entr\
                     ee", restaurant=restaurant1)

session.add(menuItem7)
session.commit()

menuItem8 = MenuItem(user_id=1, name="Veggie Burger", description="Made\
                     with freshest of ingredients and home grown spices",
...
menuItem1 = MenuItem(user_id=1, name="Chicken Stir Fry", description="With\
                     your choice of noodles vegetables and sauces",
...
menuItem2 = MenuItem(user_id=1, name="Peking Duck",
                     ...
                     sliced in front of the diners by the \
                     cook", price="$25", course="Entr\
                     ee", restaurant=restaurant2)

session.add(menuItem2)
session.commit()

menuItem3 = MenuItem(user_id=1, name="Spicy Tuna Roll", description="Seared\
                     rare ahi, avocado, edamame, cucumber with wasabi\
                     soy sauce ",
...
menuItem5 = MenuItem(user_id=1, name="Beef Noodle Soup", description="A \
                     Chinese noodle soup made of stewed or red braised\
                     beef, beef broth, vegetables and Chinese noodles.",
                     price="14", course="Entree", restaurant=restaurant2)

session.add(menuItem5)
session.commit()

menuItem6 = MenuItem(user_id=1, name="Ramen", description="a Japanese \
                     noodle soup dish. It consists of Chinese-style wheat\
                     noodles served in a meat- or (occasionally) fish-based\
                     broth, often flavored with soy sauce or miso, and uses\
                     toppings such as sliced pork, dried seaweed, kamaboko, \
                     and green onions.",
                     price="12", course="Entree", restaurant=restaurant2)
...
menuItem1 = MenuItem(user_id=1, name="Pho", description="a Vietnamese noodle \
                     soup consisting of broth, linguine-shaped rice noodles \
                     called banh pho, a few herbs, and meat.",
...
menuItem2 = MenuItem(user_id=1, name="Chinese Dumplings", description="a \
                     common Chinese dumpling which generally consists of\
                     minced meat and finely chopped vegetables wrapped into a\
                     piece of dough skin. The skin can be either thin\
                     and elastic or thicker.",
...
menuItem4 = MenuItem(user_id=1, name="Stinky Tofu", description="Taiwanese \
                     dish, deep fried fermented tofu served with \
                     pickled cabbage.",
...
menuItem1 = MenuItem(user_id=1, name="Tres Leches Cake", description="Rich, \
                     luscious sponge cake soaked in sweet milk and topped with\
...
menuItem2 = MenuItem(user_id=1, name="Mushroom risotto", description="Portabell\
                     o mushrooms in a creamy risotto",
                     price="$5.99", course="Entree", restaurant=restaurant1)
...
menuItem3 = MenuItem(user_id=1, name="Honey Boba Shaved Snow",
                     ...
                     tea jelly, grass jelly, caramel, cream, and freshly made \
                     mochi", price="$4.50", course="Dess\
                     ert", restaurant=restaurant1)

session.add(menuItem3)
session.commit()

menuItem4 = MenuItem(user_id=1, name="Cauliflower Manchurian", description="G\
                     olden fried cauliflower florets in a midly spiced soya,\
...
menuItem5 = MenuItem(user_id=1, name="Aloo Gobi Burrito", description="Vegan\
                     goodness. Burrito filled with rice, garbanzo beans, curry\
                     sauce, potatoes (aloo), fried cauliflower (gobi) and \
                     chutney. Nom Nom",
...
menuItem1 = MenuItem(user_id=1, name="Shellfish Tower", description="Lobster, \
...
menuItem2 = MenuItem(user_id=1, name="Chicken and Rice", description="Chicken\
                     and rice",
...
menuItem3 = MenuItem(user_id=1, name="Mom's Spaghetti", description="Spaghetti\
                     with some incredible tomato sauce made by mom",
                     price="$6.95", course="Entree", restaurant=restaurant1)

session.add(menuItem3)
session.commit()

menuItem4 = MenuItem(user_id=1, name="Choc Full O\' Mint (Smitten\'s Fresh Mint\
                     Chip ice cream)",
                     description="Milk, cream, salt, ..., Liquid nitrogen\
                     magic", price="$3.95", course="Dess\
                     ert", restaurant=restaurant1)
...
menuItem1 = MenuItem(user_id=1, name="Lamb Curry", description="Slow cook that \
                     thang in a pool of tomatoes, onions and alllll those \
...
menuItem9 = MenuItem(user_id=1, name="Chicken Fried Steak",
                     ...
                     smothered with cream gravy", price="$8.99", course="Ent\
                     ree", restaurant=restaurant1)
...
menuItem1 = MenuItem(user_id=1, name="Boysenberry Sorbet", description="An \
                     unsettlingly huge amount of ripe berries turned into \
...
menuItem2 = MenuItem(user_id=1, name="Broiled salmon", description="Salmon \
                     fillet marinated with fresh herbs and broiled hot & fast",
...
menuItem3 = MenuItem(user_id=1, name="Morels on toast (seasonal)",
                     ...
                     served on herbed toast slices", price="$7.50", course="A\
                     ppetizer", restaurant=restaurant1)

session.add(menuItem3)
session.commit()

menuItem4 = MenuItem(user_id=1, name="Tandoori Chicken", description="Chicken\
                     marinated in yoghurt and seasoned with a spicy mix\
                     (chilli, tamarind among others) and slow cooked in a \
                     cylindrical clay or metal oven which gets its heat from\
                     burning charcoal.",
...
menuItem10 = MenuItem(user_id=1, name="Spinach Ice Cream", description="vanilla\
                      ice cream made with organic spinach leaves",
...
menuItem1 = MenuItem(user_id=1, name="Super Burrito Al Pastor",
                     ...
                     Cilantro, Salsa, Tortilla", price="$5.95", course="Entr\
                     ee", restaurant=restaurant1)
...
menuItem2 = MenuItem(user_id=1, name="Cachapa", description="Golden brown, \
                     corn-based Venezuelan pancake; usually stuffed with \
...
menuItem1 = MenuItem(user_id=1, name="Chantrelle Toast", description="Crispy\
                     Toast with Sesame Seeds slathered with buttery \
...
menuItem1 = MenuItem(user_id=1, name="Guanciale Chawanmushi",
                     ...
                     Italian Pork Jowl (guanciale)", price="$6.95", course="De\
                     ssert", restaurant=restaurant1)
...
menuItem1 = MenuItem(user_id=1, name="Lemon Curd Ice Cream Sandwich",
                     description="Lemon Curd Ice Cream Sandwich on a chocolate\
                     macaron with cardamom meringue and \
                     cashews", price="$4.25", course="Dess\
                     ert", restaurant=restaurant1)
